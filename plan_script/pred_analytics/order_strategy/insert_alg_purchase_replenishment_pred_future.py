...
sql_insert = mysql_sql['insert_alg_purchase_replenishment_pred_future']
sql_insert_value = mysql_sql['val_alg_purchase_replenishment_pred_future']

val_data = ''
for wh in wh_dept_list:
    for spec in goods_spec_list:
        val_str = (sql_insert_value + ',\n').format(
            wh['wh_dept_id'], spec['spec_id']
        )
        val_data += val_str
...
with open('../data/order_strategy/alg_purchase_replenishment_pred_future.txt', 'w', encoding='utf-8') as f:
    f.write(insert_data)
