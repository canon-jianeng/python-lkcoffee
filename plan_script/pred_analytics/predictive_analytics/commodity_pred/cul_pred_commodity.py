...
日纬度 -> 周纬度
商品明细 -> 商品总量
{wh: 1, commodity: 1, dt: 1, value: 1, shop: 1}
...
# 查询仓库
wh_data = []
# 查询商品 t_prediction_cup_commodity
commodity_data = []

pred_dict = {}
for predict_dt in predict_dt_list:
    for wh_id in wh_data:
        for commodity_id in commodity_data:
            if datetime.strptime(predict_dt, '%Y-%m-%d').date() < now_date:
                shop_num = 0

                cursor.execute(sql_actual_cup_commodity.format(commodity_id, predict_dt, wh_id))
                data = cursor.fetchall()
                # 查询实际（过去）商品杯量
                if data == ():
                    pred_dict.update(
                        {'wh': wh_id, 'commodity': '', 'cup_num': '', 'dt': predict_dt, 'shop_num': shop_num})
                else:
                    for value in data:
                        pred_dict.update(
                            {'wh': wh_id, 'commodity': '', 'cup_num': value[0], 'dt': predict_dt, 'shop_num': shop_num})
            else:
                shop_num = 0

                cursor.execute(sql_pred_cup_commodity.format(commodity_id, version_val, predict_dt, wh_id))
                data = cursor.fetchall()
                # 查询预测（未来）商品杯量
                if data == ():
                    pred_dict.update(
